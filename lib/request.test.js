...
describe('sendRequest', () => {
  ...
  it('makes a GET request by default', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith('/YamahaExtendedControl/v1/my/path', { method: 'GET' });
  });

  it('makes a POST request when params are supplied', async () => {
    await sendRequest('my/path', { foo: 'bar' });
    expect(fetch).toHaveBeenCalledWith('/YamahaExtendedControl/v1/my/path', { method: 'POST', body: '{"foo":"bar"}' });
  });
